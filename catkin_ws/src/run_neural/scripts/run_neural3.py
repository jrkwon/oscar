...
cur_target_rc_yaw = AttitudeTarget()
...
local_target_pub = rospy.Publisher('/mavros/setpoint_raw/attitude', AttitudeTarget, queue_size=100)
def RCInOverride(thrt,vx,vy):
	target_RC_yaw = AttitudeTarget()
	target_RC_yaw.thrust = thrt
	target_RC_yaw.orientation.x = vx
	target_RC_yaw.orientation.y = vy
...
def main(weight_file_name):
    ...
    if Config.data_collection['vehicle_name'] == 'rover':
        joy_pub4mavros = rospy.Publisher(Config.data_collection['mavros_cmd_vel_topic'], Twist, queue_size=20)
    ...
    while not rospy.is_shutdown():
        ...
        cur_target_rc_yaw = RCInOverride(thrt,vx,vy)
        ##############################    
        ...
        if Config.data_collection['vehicle_name'] == 'rover':
            ...
            if neural_control.braking is True:
                ...
                thrt = joy_data.throttle*Config.run_neural['scale_factor_throttle']
                vx = joy_data.steer*Config.run_neural['scale_factor_steering']
        
            global cur_target_rc_yaw
            cur_target_rc_yaw = RCInOverride(thrt,vx,vy)
            joy_pub4mavros.publish(joy_data4mavros)
            local_target_pub.publish(target_RC_yaw)
...
if __name__ == "__main__":
    try:
        if len(sys.argv) != 2:
            exit('Usage:\n$ rosrun run_neural run_neural3.py weight_file_name')

        main(sys.argv[1])
        cur_target_rc_yaw = RCInOverride(0,0,0)
