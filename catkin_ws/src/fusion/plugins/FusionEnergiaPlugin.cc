...
#include "FusionEnergiaPlugin.hh"
...
namespace gazebo
{
  class FusionEnergiaPluginPrivate
  {
    /// \enum DirectionType
...
/////////////////////////////////////////////////
FusionEnergiaPlugin::FusionEnergiaPlugin()
    : dataPtr(new FusionEnergiaPluginPrivate)
{
  int argc = 0;
  char *argv = nullptr;
  ros::init(argc, &argv, "FusionEnergiaPlugin");
  ros::NodeHandle nh;
  this->dataPtr->controlSub = nh.subscribe("fusion", 10, &FusionEnergiaPlugin::OnPriusCommand, this);

  this->dataPtr->directionState = FusionEnergiaPluginPrivate::FORWARD;
...
void FusionEnergiaPlugin::OnPriusCommand(const fusion::Control::ConstPtr &msg)
{
  ...
  {
    case fusion::Control::NEUTRAL:
      this->dataPtr->directionState = FusionEnergiaPluginPrivate::NEUTRAL;
      break;
    case fusion::Control::FORWARD:
      this->dataPtr->directionState = FusionEnergiaPluginPrivate::FORWARD;
      break;
    case fusion::Control::REVERSE:
      this->dataPtr->directionState = FusionEnergiaPluginPrivate::REVERSE;
      break;
    default:
      break;
  }
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  gzwarn << "FusionEnergiaPlugin loading params" << std::endl;
  // shortcut to this->dataPtr
  FusionEnergiaPluginPrivate *dPtr = this->dataPtr.get();
  ...
  this->dataPtr->node.Subscribe("/prius/reset",
      &FusionEnergiaPlugin::OnReset, this);
  this->dataPtr->node.Subscribe("/prius/stop",
      &FusionEnergiaPlugin::OnStop, this);

  this->dataPtr->node.Subscribe("/cmd_vel", &FusionEnergiaPlugin::OnCmdVel, this);
  this->dataPtr->node.Subscribe("/cmd_gear",
      &FusionEnergiaPlugin::OnCmdGear, this);
  this->dataPtr->node.Subscribe("/cmd_mode",
      &FusionEnergiaPlugin::OnCmdMode, this);
  ...
  this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(
      std::bind(&FusionEnergiaPlugin::Update, this));

  this->dataPtr->keyboardSub =
    this->dataPtr->gznode->Subscribe("~/keyboard/keypress",
        &FusionEnergiaPlugin::OnKeyPress, this, true);

  this->dataPtr->worldControlPub =
    this->dataPtr->gznode->Advertise<msgs::WorldControl>("~/world_control");

  this->dataPtr->node.Subscribe("/keypress", &FusionEnergiaPlugin::OnKeyPressIgn,
      this);
}

/////////////////////////////////////////////////
void FusionEnergiaPlugin::OnCmdVel(const ignition::msgs::Pose &_msg)
{
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::OnCmdGear(const ignition::msgs::Int32 &_msg)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  this->dataPtr->directionState =
      static_cast<FusionEnergiaPluginPrivate::DirectionType>(state);
}

/////////////////////////////////////////////////
void FusionEnergiaPlugin::OnCmdMode(const ignition::msgs::Boolean &/*_msg*/)
{
  // toggle ev mode
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::KeyControlTypeA(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::KeyControlTypeB(const int _key)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  switch (_key)
  {
    ...
    {
      this->dataPtr->brakePedalPercent = 0.0;
      if (this->dataPtr->directionState != FusionEnergiaPluginPrivate::REVERSE)
        this->dataPtr->gasPedalPercent = 0.0;
      this->dataPtr->gasPedalPercent += 0.1;
      this->dataPtr->gasPedalPercent =
          std::min(this->dataPtr->gasPedalPercent, 1.0);
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::KeyControl(const int _key)
{
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::OnKeyPress(ConstAnyPtr &_msg)
{
  this->KeyControl(_msg->int_value());
}

/////////////////////////////////////////////////
void FusionEnergiaPlugin::OnKeyPressIgn(const ignition::msgs::Any &_msg)
{
  this->KeyControl(_msg.int_value());
}

/////////////////////////////////////////////////
void FusionEnergiaPlugin::OnReset(const ignition::msgs::Any & /*_msg*/)
{
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::OnStop(const ignition::msgs::Any & /*_msg*/)
{
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::Reset()
{
  this->dataPtr->odom = 0;
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::Update()
{
  // shortcut to this->dataPtr
  FusionEnergiaPluginPrivate *dPtr = this->dataPtr.get();
  ...
  bool neutral = dPtr->directionState == FusionEnergiaPluginPrivate::NEUTRAL;
  ...
  if ((curTime - this->dataPtr->lastMsgTime) > .5)
  {
    ...
    if (this->dataPtr->directionState == FusionEnergiaPluginPrivate::FORWARD)
      consoleMsg.add_data(1.0);
    else if (this->dataPtr->directionState == FusionEnergiaPluginPrivate::REVERSE)
      consoleMsg.add_data(2.0);
    else if (this->dataPtr->directionState == FusionEnergiaPluginPrivate::NEUTRAL)
...
/////////////////////////////////////////////////
void FusionEnergiaPlugin::UpdateHandWheelRatio()
{
...
double FusionEnergiaPlugin::CollisionRadius(physics::CollisionPtr _coll)
{
...
/////////////////////////////////////////////////
double FusionEnergiaPlugin::GasTorqueMultiplier()
{
...
GZ_REGISTER_MODEL_PLUGIN(FusionEnergiaPlugin)
