...
import rospy
from mavros_msgs.msg import OverrideRCIn, State, AttitudeTarget
...
local_target_pub1 = rospy.Publisher('/mavros/setpoint_raw/attitude', AttitudeTarget, queue_size=30)
local_target_pub = rospy.Publisher('/mavros/setpoint_velocity/cmd_vel_unstamped', Twist, queue_size=100)
def __init__():
	rospy.init_node('PX4_control' ,anonymous=True)
	rospy.Subscriber("/mavros/state", State, mavros_state_callback)
	print("Initialized")
def RCInOverride(thrt,vx):
	target_RC_yaw = Twist()
	target_RC_yaw.linear.y = thrt
	target_RC_yaw.linear.x = vx
	#target_RC_yaw.linear.y = vy
...
def action_control():
	...
	if key == 'e' or key == 'E':
		...
		thrt = speed_control+0.1
	else:
		thrt = 0
	#roll
	if key == 's' or key == 'S':
		vx = speed_control+0.1
	elif key == 'f' or key == 'F':
		vx = speed_control
	else:
		vx = 0
	...
	cur_target_rc_yaw = RCInOverride(thrt,vx)
	if key == 'h' or key == 'H':
...
if __name__=="__main__":
	...
	print (msg)
	__init__()
	cur_target_rc_yaw = RCInOverride(0,0)
	while(1):
		key= getKey()
		command_control()
		action_control()
		local_target_pub1.publish()
