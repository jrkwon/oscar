...
class Translator:
    def __init__(self):
        ...
        #self.pub4mavros = rospy.Publisher('/mavros/setpoint_velocity/cmd_vel_unstamped', Twist, queue_size=100)
        self.pub4mavros2 = rospy.Publisher('/mavros/setpoint_raw/attitude', AttitudeTarget, queue_size=20)
    ...
    def callback(self, message):
        ...
        command4mavros2 = AttitudeTarget()
        if command.shift_gears == Control.FORWARD:
            command4mavros2.thrust = command.throttle
            command4mavros2.orientation.x = command.steer
            #command4mavros.linear.x = command.steer
        elif command.shift_gears == Control.REVERSE:
            command4mavros2.thrust = -command.throttle
            command4mavros2.orientation.y = command.steer
        else:
            command4mavros2.thrust = command.throttle
            if command.steer >= 0:
            	command4mavros2.orientation.x = command.steer
        ...
        #self.pub4mavros2.publish(command4mavros2)
...
if __name__ == '__main__':
    rospy.init_node('joystick_translator2a')
    t = Translator()
    rospy.spin()
